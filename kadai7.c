  ...
  while (1) {
    ...
    if(topContoursInfo[0].area > 0)
    {
      ...
      // 垂直二等分線の角度を計算 if 垂直二等分線上に乗っていない場合、、、 (*)までif文の中の処理
        ...
        motor(128-20, 128+20); // 回転、今は一旦左回転
        usleep(/*時間指定*/); // 回転角度によって時間を変更
        motor(148,　160);　// 直進
        usleep(/*時間指定*/); //時間に距離によって変更
        motor(128+20, 128-20);
        usleep(/*時間指定*/); // 回転角度によって時間を変更
        // (*)
      ...
      // 見えなくなってからの処理！
      if (c >= 268) {                                      // y座標で制御
        sleep(2.656*100000);                               // ()の中に時間(sec)を書く。ここで38.5cm直進をする。 ただこの値は(*)によって変わってくる。
        motor(128, 128);                                   // 停止
      }
    }
    else
